...
class PipelineExecutor
{
    ...
    public function execute(array $pipeline, Collection $data): array
    {
        ...
        foreach ($pipeline as $step) {
            ...
            if ($result === null) {
                // null propagation — downstream receives null collection
                $result = collect();
            }
        }
        ...
        // Action is triggered if compare produced results or any rows remain and no compare
    ...
    private function applyAggregate(array $step, Collection $groups): Collection
    {
        ...
        // If data is already grouped (Collection of Collections), aggregate per group
        ...
        // Single aggregate over entire dataset
    ...
    private function applyMovingAvg(array $step, Collection $data): Collection
    {
        ...
        // If grouped, compute per group independently
    ...
    private function computeMovingAvgOnGroup(Collection $group, string $metric, int $windowDays, int $minPeriods, string $alias): Collection
    {
        ...
        foreach ($rows as $i => &$row) {
            // Simple window: take up to $windowDays prior rows
