...
use App\Services\AiSpecGenerator;
use App\Services\SemanticValidator;
...
class FeatureController extends Controller
{
    ...
    public function rollback(Request $request, string $id): JsonResponse
    {
        ...
        $snapshot = \App\Models\FeatureSnapshot::where('id', $request->input('snapshot_id'))
            ->where('tenant_id', $tenant->id)
            ->firstOrFail();

        // Find the feature state in the snapshot
    ...
    {
        // In production: resolved from authenticated user's tenant relationship
        // For now: resolved from X-Tenant-ID header (replace with proper auth middleware)
